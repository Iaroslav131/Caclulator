...
class Calculator(QMainWindow):
    ...
    def is_math_error(self) -> bool:
        return self.ui.le_entry.text() == "MATH ERROR"

    def add_digit(self, btn_text: str) -> None:
        if self.is_math_error():
    ...
    def add_temp(self, math_sign: str):
        if not self.ui.lbl_temp.text() or self.get_math_sign() == '=':
            self.ui.lbl_temp.setText(self.remove_trailing_zeroes(self.ui.le_entry.text()) + f' {math_sign}')
            self.ui.le_entry.setText('0')
    ...
    def calculate(self) -> Optional[str]:
        ...
        if temp:
            try:
                ...
                self.ui.lbl_temp.setText(temp + self.remove_trailing_zeroes(entry) + '=')
                self.ui.le_entry.setText(result)
                return result
            except (ZeroDivisionError, OverflowError, ValueError):
                self.ui.le_entry.setText("MATH ERROR")
                self.ui.lbl_temp.clear()
        return None

    def math_operation(self, math_sign: str) -> Optional[str]:
        if self.is_math_error():
            return
        temp = self.ui.lbl_temp.text()
